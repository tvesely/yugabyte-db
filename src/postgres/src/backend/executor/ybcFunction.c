...
void
YbSetSRFTargets(YBCPgFunction handle, TupleDesc desc)
{
	for (int attr_num = 0; attr_num < desc->natts; ++attr_num)
	{
		...
		HandleYBStatus(
			YBCAddFunctionTarget(handle, attr_name, type_entity, type_attrs));
	}
	HandleYBStatus(YBCFinalizeFunctionTargets(handle));
}

bool
YbSRFGetNext(YBCPgFunction handle, uint64_t *values, bool *is_nulls)
{
	bool has_data = false;
	HandleYBStatus(YBCSRFGetNext(handle, values, is_nulls, &has_data));
	return has_data;
}